...
X_train, y_train = dataset.create_dataset(x_grid.reshape(-1, 1), t_grid.reshape(-1, 1), n_samples=0, noise=0.3, random=True, return_idx=False, random_state=42) # use the same dataset for every run; only diff is in the network
...
train_dynamic(model, X_train, y_train, optimizer, 10000, stopper_kwargs={'ini_epoch': 1000, 'patience': 50}, log_dir=f'testruns/threshold/')
