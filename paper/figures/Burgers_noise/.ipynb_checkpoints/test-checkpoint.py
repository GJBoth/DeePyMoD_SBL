...
for noise_level in [0.8]:
    for run in np.arange(n_runs):
        X_train, y_train = dataset.create_dataset(x_grid.reshape(-1, 1), t_grid.reshape(-1, 1), n_samples=1000, noise=noise_level, random=True, return_idx=False, random_state=42) # use the same dataset for every run; only diff is in the network
        ...
        train_dynamic(model, X_train, y_train, optimizer, 6000, log_dir=f'testruns/new_stop_cluster_run_{run}/')
