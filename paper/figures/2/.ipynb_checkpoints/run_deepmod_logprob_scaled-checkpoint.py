...
def train(model, data, target, optimizer, max_iterations, loss_func_args, log_dir=None):
    ...
    for iteration in torch.arange(0, max_iterations + 1):
        ...
        if iteration % 100 == 0:
            ...
            board.write(iteration, loss, loss_mse, loss_reg, loss_ll, coeff_vector_list, coeff_vector_scaled_list, log_likelihood=loss_ll, ll_fit=loss_ll_fit, lstsq_solution=lstsq_solution.squeeze(), mae_library=mae_library, mae_time_deriv=mae_dt, lstsq_solution_normed=lstsq_solution_normed.squeeze())
...
for run_idx in np.arange(n_runs):
    ...
    train(model, X_train, y_train, optimizer, 20000, loss_func_args={'library':torch.tensor(theta) ,'time_deriv': torch.tensor(dt)}, log_dir = f'runs_new/deepmod_logprob_run_{run_idx}')
    torch.save(model.state_dict(), f'data/deepmod_logprob_run_{run_idx}.pt')