...
noise_range = np.arange(0.0, 1.01, 0.1)
n_runs = 5

for noise_level in noise_range:
    for run in np.arange(n_runs):
        X_train, y_train = dataset.create_dataset(X, t_grid.reshape(-1, 1), n_samples=5000, noise=noise_level, random=True, return_idx=False, random_state=42) # use the same dataset for every run; only diff is in the network
        estimator = Clustering(estimator=LassoLarsIC(fit_intercept=False))
        config = {'n_in': 3, 'hidden_dims': [30, 30, 30, 30, 30], 'n_out': 1, 'library_function':library_2Din_1Dout, 'library_args':{'poly_order': 1, 'diff_order': 3}, 'sparsity_estimator': estimator}
        model = DeepModDynamic(**config)
        optimizer = torch.optim.Adam(model.parameters(), betas=(0.99, 0.99), amsgrad=True)
        train_dynamic(model, X_train, y_train, optimizer, 15000, stopper_kwargs={'ini_epoch': 1000, 'patience': 50}, log_dir=f'runs/cluster_{noise_level:.2f}_run_{run}/')
