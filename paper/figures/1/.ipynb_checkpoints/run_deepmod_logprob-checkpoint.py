...
def train(model, data, target, optimizer, max_iterations, log_dir=None):
    ...
    for iteration in torch.arange(0, max_iterations + 1):
        ...
        if iteration % 100 == 0:
            ...
            board.write(iteration, loss, loss_mse, loss_reg, loss_ll, coeff_vector_list, coeff_vector_scaled_list, log_likelihood=loss_ll)
...
for run_idx in np.arange(n_runs):
    ...
    train(model, X_train, y_train, optimizer, 20000, log_dir = f'runs/deepmod_logprob_run_{run_idx}')
    torch.save(model.state_dict(), f'data/deepmod_log_prob_run_{run_idx}.pt')