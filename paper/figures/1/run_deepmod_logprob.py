...
def train(model, data, target, optimizer, max_iterations, loss_func_args, log_dir=None):
    ...
    for iteration in torch.arange(0, max_iterations + 1):
        ...
        loss = torch.sum(loss_ll) + torch.sum(loss_reg)
        ...
        if iteration % 100 == 0:
            ...
            board.write(iteration, loss, loss_mse, loss_reg, loss_reg, coeff_vector_list, coeff_vector_scaled_list, lstsq_solution=lstsq_solution.squeeze(), mae_library=mae_library, mae_time_deriv=mae_dt, log_likelihood=loss_ll)
