...
for run_idx in np.arange(n_runs):
    ...
    train(model, X_train, y_train, optimizer, 100000, loss_func_args={'library':torch.tensor(theta) ,'time_deriv': torch.tensor(dt)}, log_dir = f'runs/deepmod_logprob_lstsq_run_{run_idx}')
